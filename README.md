...
Each tool is a `fetch` to the MCP server using JSON-RPC `tools/call`; the client validates that a tool name is present before sending a request. Successful HTTP response bodies are validated as JSON-RPC responses before their MCP content is interpreted. Multi-step tool chains are allowed (up to 5 steps), and the result streams back with `toUIMessageStreamResponse()`, so the UI sees tool inputs and outputs as typed message parts.
